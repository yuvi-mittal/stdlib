...
tape('the function returns NaN if provided NaN as any argument', opts , function test(t) {
	var y;
...
tape('the function returns NaN for invalid sigma values (sigma <= 0)', opts , function test(t) {
	var y;
...
tape('the function returns NaN when a >= b', opts , function test(t) {
	var y;
...
tape('the function returns 0 when x is out of range (x < a or x > b)', opts , function test(t) {
	var y;
...
tape('the function evaluates the pdf', opts , function test(t) {
	...
	for (i = 0; i < x.length; i++) {
		...
		if (y === expected[i]) {
			...
			t.ok(delta <= tol, `within tolerance. x: ${x[i]}, mu: ${mu[i]}, sigma: ${sigma[i]}, a: ${a[i]}, b: ${b[i]}, y: ${y}, expected: ${expected[i]}, Δ: ${delta}, tol: ${tol}`);
		}
	}
	t.end();
});
