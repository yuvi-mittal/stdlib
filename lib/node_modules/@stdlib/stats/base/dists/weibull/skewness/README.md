...
Calculates the skewness for a Weibull distribution.

```c
double out = stdlib_base_dists_weibull_skewness( 2.0, 3.0 );
// returns ~0.087
```

The function accepts the following arguments:

-   **k**: `[in] double` shape parameter (k > 0).
-   **lambda**: `[in] double` scale parameter (lambda > 0).
