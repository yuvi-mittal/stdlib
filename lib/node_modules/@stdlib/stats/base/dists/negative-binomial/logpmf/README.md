...
The C interface for evaluating the natural logarithm of the probability mass function (PMF) for a negative binomial distribution.
...
```c
#include "stdlib/stats/base/dists/negative_binomial/logpmf.h"
```

#### stdlib_base_dists_negative_binomial_logpmf( x, r, p )

Evaluates the natural logarithm of the probability mass function (PMF) for a negative binomial distribution with the number of successes until the experiment is stopped `r` and success probability `p`.
...
-   **r**: `[in] double` number of successes until the experiment is stopped.
...
-   Returns `NaN` if any argument is `NaN`, if `r <= 0`, or if `p` is outside the interval `[0, 1]`.
-   Returns negative infinity (`-∞`) if `x` is not a nonnegative integer or if `p == 0`.
...
// Generate a random uniform value between two bounds:
...
int main( void ) {
    ...
    // Generate random inputs and evaluate the logPMF:
    for ( i = 0; i < 25; i++ ) {
        ...
        printf( "x: %lf, r: %lf, p: %lf, logPMF(x;r,p): %lf\n", x, r, p, y );
    }
}
```
